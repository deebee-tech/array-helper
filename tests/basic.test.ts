...
describe("comparison semantics", () => {
   it("should sort Dates chronologically, not by their string form", () => {
      ...
      const arr = [{ at: new Date("2026-02-20") }, { at: new Date("2026-01-05") }, { at: new Date("2026-03-10") }];

      const result = orderBy(arr, ["at"]);

      expect(result.map((r) => r.at.toISOString().slice(0, 10))).toEqual(["2026-01-05", "2026-02-20", "2026-03-10"]);
   });
   ...
   it("should sort nullish values last ascending and first descending", () => {
      ...
      expect(orderBy(arr, ["v"], ["desc"]).map((r) => r.v)).toEqual([null, undefined, "zebra", "apple"]);
   });
